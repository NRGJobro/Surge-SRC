...
void ChestESP::onPreRender(C_MinecraftUIRenderContext* renderCtx) {
	...
	for (auto iter = bufferedChestList.begin(); iter != bufferedChestList.end(); ++iter) {
		...
		if (storageID == 54) DrawUtils::setColor(191.f / 0.f, 0.f / 0.f, 0.f / 255.f, sexyMathHack);
		...
		DrawUtils::drawBox((*iter)->upper.floor().add(mathVect), (*iter)->upper, (float)fmax(0.2f, 1 / (float)fmax(1, g_Data.getLocalPlayer()->eyePos0.dist((*iter)->lower))), true);  // An illusion of good esp
	}
}
