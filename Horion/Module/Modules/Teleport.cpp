...
Teleport::Teleport() : IModule(0, Category::MOVEMENT, "Click a block to and sneak to teleport to it. (Bypass dosent work)") {
...
void Teleport::onTick(C_GameMode* gm) {
	...
	if (GameData::isRightClickDown() && !hasClicked) {
		...
		int rayType = g_Data.getClientInstance()->getPointerStruct()->rayHitType;
		if (rayType != 0) return;
	...
	if (shouldTP && GameData::isKeyDown(VK_SHIFT)) {
		tpPos.y += (gm->player->getPos()->y - gm->player->getAABB()->lower.y) + 1;  // eye height + 1
		if (bypass) {
			int dist2 = (int)gm->player->getPos()->dist(tpPos);
			int i = (int)dist2 / 5;
			for (int n = 0; n < i; n++) {
