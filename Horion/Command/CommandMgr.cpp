...
void CommandMgr::initCommands() {
	...
	commandList.push_back(new YTCommand());
...
void CommandMgr::execute(char* message) {
	if (message != nullptr) {
		...
		g_Data.getClientInstance()->getGuiData()->displayClientMessageF("[%Surge%s] %sCommand '%s' could not be found!", GOLD, WHITE, RED, cmd.c_str());
	}
}
