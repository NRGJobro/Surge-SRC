...
// HUGE CREDIT TO matrix for the injector.
Hooks g_Hooks;
bool isTicked = false;
...
void Hooks::Actor_lerpMotion(C_Entity* _this, vec3_t motVec) {
	...
	if (noKnockbackmod->isEnabled()) {
		...
		if (networkSender == _ReturnAddress()) {
			motVec = _this->velocity.lerp(motVec, 0.0);
		}
	}
...
void Hooks::JumpPower(C_Entity* a1, float a2) {
	...
	if (LongJumpMod->isEnabled() && g_Data.getLocalPlayer() == a1) {
		a1->velocity.y = LongJumpMod->jumpPower;
		return;
	}
	oFunc(a1, a2);
}
